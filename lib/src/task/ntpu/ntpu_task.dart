...
class NTPU_Task<T> extends DialogTask<T> {
  static bool _isLogin = false;

  NTPU_Task(name) : super("NTPUTask $name");
  ...
  Future<TaskStatus> execute() async {
    ...
    try {
      super.onStart(R.current.loginNTPU);
      final loginResult = await NTPU_Connector.Login(account, password);
      super.onEnd();

      if(loginResult == true) {
        _isLogin = loginResult;
        return _handleConnectorStatus(AccountStatus.normal);
      }
    }
    catch (e, stackTrace) {
